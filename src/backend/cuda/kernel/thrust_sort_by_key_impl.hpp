...
#ifdef AF_CUDA_SEGMENTED_SORT_INSTANCE
#include <kernel/segmented_sort_impl.hpp>
#endif
...
void thrustSortByKey(Tk *keyPtr, Tv *valPtr, int elements, bool isAscending) {
    if (isAscending) {
        THRUST_SELECT_NOSYNC(thrust::stable_sort_by_key, keyPtr,
                             keyPtr + elements, valPtr);
    } else {
        THRUST_SELECT_NOSYNC(thrust::stable_sort_by_key, keyPtr,
                             keyPtr + elements, valPtr, thrust::greater<Tk>());
    }
    POST_LAUNCH_CHECK();
}
