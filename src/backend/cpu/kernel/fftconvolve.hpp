...
constexpr size_t fftconvolve_min_parallel_items = 1 << 18;
constexpr size_t fftconvolve_min_items_per_task = 1 << 16;

template<typename Function>
void forEachFFTConvolveItem(const af::dim4& dims, Function function) {
    if (getParallelThreadCount() == 1 ||
        dims.elements() < static_cast<dim_t>(fftconvolve_min_parallel_items)) {
        for (dim_t d3 = 0; d3 < dims[3]; ++d3) {
            for (dim_t d2 = 0; d2 < dims[2]; ++d2) {
                for (dim_t d1 = 0; d1 < dims[1]; ++d1) {
                    for (dim_t d0 = 0; d0 < dims[0]; ++d0) {
    ...
    parallelForEach(dims, fftconvolve_min_items_per_task, function);
}
...
void packData(Param<To> out, const af::dim4 od, const af::dim4 os,
    ...
    const dim_t id0_half = divup(id[0], 2);
    const bool odd_id0   = (id[0] % 2 == 1);

    af::dim4 pair_dims = od;
    pair_dims[0] /= 2;
    forEachFFTConvolveItem(
        pair_dims, [=](dim_t d0, dim_t d1, dim_t d2, dim_t d3) {
    ...
    forEachFFTConvolveItem(
        pair_dims, [=](dim_t d0, dim_t d1, dim_t d2, dim_t d3) {
            ...
            if (d0 < id[0] && d1 < id[1] && d2 < id[2] && d3 < id[3]) {
                // Copy input elements to real elements, set imaginary elements
                // to 0
                const dim_t iidx  = d3 * is[3] + d2 * is[2] + d1 * is[1] + d0;
                out_ptr[oidx]     = (To)in_ptr[iidx];
                out_ptr[oidx + 1] = (To)0;
...
void complexMultiply(Param<T> packed, const af::dim4 sig_dims,
    ...
    af::dim4 pair_dims = od;
    pair_dims[0] /= 2;
    forEachFFTConvolveItem(pair_dims, [=](dim_t d0, dim_t d1, dim_t d2,
                                          dim_t d3) {
        if (kind == AF_BATCH_NONE || kind == AF_BATCH_SAME) {
            // Complex multiply each signal to equivalent filter
            const dim_t ridx = d3 * os[3] + d2 * os[2] + d1 * os[1] + d0 * 2;
            const dim_t iidx = ridx + 1;
            ...
            const dim_t ridx1 = d3 * os[3] + d2 * os[2] + d1 * os[1] + d0 * 2;
            const dim_t iidx1 = ridx1 + 1;
            const dim_t ridx2 = ridx1 % (i2s[3] * i2d[3]);
            const dim_t iidx2 = iidx1 % (i2s[3] * i2d[3]);
            ...
            const dim_t ridx2 = d3 * os[3] + d2 * os[2] + d1 * os[1] + d0 * 2;
            const dim_t iidx2 = ridx2 + 1;
            const dim_t ridx1 = ridx2 % (i1s[3] * i1d[3]);
            const dim_t iidx1 = iidx2 % (i1s[3] * i1d[3]);
...
void reorderHelper(To* out_ptr, const af::dim4& od, const af::dim4& os,
                   ...
                   const af::dim4& fd, const dim_t half_di0,
                   const dim_t fftScale) {
    constexpr bool RoundResult = std::is_integral<To>::value;

    UNUSED(id);
    forEachFFTConvolveItem(od, [=](dim_t d0, dim_t d1, dim_t d2, dim_t d3) {
        dim_t id0, id1, id2, id3;
        if (Expand) {
            id0 = d0;
            id1 = d1 * is[1];
            id2 = d2 * is[2];
        ...
        const dim_t oidx = d3 * os[3] + d2 * os[2] + d1 * os[1] + d0;
        ...
        if (id0 < half_di0) {
            // Copy top elements
            const dim_t iidx = id3 + id2 + id1 + id0 * 2;
            if (RoundResult)
            ...
            const dim_t iidx1 = id3 + id2 + id1 + id0 * 2;
            const dim_t iidx2 = id3 + id2 + id1 + (id0 - half_di0) * 2 + 1;
            if (RoundResult)
                out_ptr[oidx] = (To)roundf(
                    (float)((in_ptr[iidx1] + in_ptr[iidx2]) / fftScale));
            else
                out_ptr[oidx] =
                    (To)((in_ptr[iidx1] + in_ptr[iidx2]) / fftScale);
        } else {
            // Copy bottom elements
            const dim_t iidx = id3 + id2 + id1 + (id0 - half_di0) * 2 + 1;
            if (RoundResult)
                out_ptr[oidx] = (To)roundf((float)(in_ptr[iidx] / fftScale));
            else
                out_ptr[oidx] = (To)(in_ptr[iidx] / fftScale);
